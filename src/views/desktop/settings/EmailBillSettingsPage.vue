                ...
                <v-window-item value="classification">
                    <v-card>
                        ...
                        <v-table><thead><tr><th>{{ tt('Merchant Pattern') }}</th><th>{{ tt('Match Type') }}</th><th>{{ tt('Category') }}</th><th>{{ tt('Source') }}</th><th>{{ tt('Status') }}</th><th class="text-right">{{ tt('Actions') }}</th></tr></thead>
                            <tbody><tr v-for="rule in store.classifications" :key="rule.id"><td>{{ rule.merchantPattern }}</td><td>{{ rule.matchType }}</td><td>{{ categoryName(rule.categoryId) }}</td><td><v-chip size="small" variant="tonal">{{ rule.origin }}</v-chip></td><td>{{ rule.enabled ? tt('Enabled') : tt('Disabled') }}</td><td class="text-right"><v-btn size="small" variant="text" @click="openClassification(rule)">{{ tt('Edit') }}</v-btn><v-btn v-if="rule.enabled" size="small" color="error" variant="text" @click="disableClassification(rule.id)">{{ tt('Disable') }}</v-btn></td></tr><tr v-if="!store.classifications.length"><td colspan="6" class="text-center text-medium-emphasis py-8">{{ tt('No classification rules') }}</td></tr></tbody>
                        </v-table>
                    </v-card>
                </v-window-item>
